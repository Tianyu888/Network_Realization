...
class MainWidget(QWidget, ui1.Ui_Form):
    ...
    def init_bartool(self):
        # Attack Mode
        self.Drag.setStyleSheet("background-image : url(/Users/shitianyu/Desktop/urpn/pics/drag.png);")
        ...
        self.Color.setStyleSheet("background-image : url(/Users/shitianyu/Desktop/urpn/pics/color.png);")
        # Initialize color button
        self.Color.clicked.connect(self.changenodecolor)
        self.Spring_Layout.setStyleSheet("background-image : url(/Users/shitianyu/Desktop/urpn/pics/layout.png);")
        # Initialize layout button
        self.Spring_Layout.clicked.connect(self.spring_layout)

        # Defend Mode
        self.Drag_2.setStyleSheet("background-image : url(/Users/shitianyu/Desktop/urpn/pics/drag.png);")
        # Initialize the drag button
        self.Drag_2.clicked.connect(self.movable)
        self.Color_2.setStyleSheet("background-image : url(/Users/shitianyu/Desktop/urpn/pics/color.png);")
        # Initialize color button
    ...
    def draw_SW_Network(self):
        ...
        if not self.directed:
            pos = dict()
            for i in range(len(SW.nodes)):
                pos[i] = (xydata_b[i][0], xydata_b[i][1])
            nx.set_node_attributes(SW, pos, 'pos')
            for i in SW.edges():
                self.matplotlibwidget.axis.plot(np.array([xydata_b[i[0]][0], xydata_b[i[1]][0]]),
                                                np.array([xydata_b[i[0]][1], xydata_b[i[1]][1]]),
                                                color="grey", zorder=1)
    ...
    def draw_ER_Network(self):
        ...
        if not self.Directed.isChecked():
            ...
            for i in range(len(ER.nodes)):
                pos[i] = (xydata_b[i][0], xydata_b[i][1])
            nx.set_node_attributes(ER, pos, 'pos')
            for i in ER.edges():
                self.matplotlibwidget.axis.plot(np.array([xydata_b[i[0]][0], xydata_b[i[1]][0]]),
                                                np.array([xydata_b[i[0]][1], xydata_b[i[1]][1]]),
                                                color="grey", zorder=1)
            ...
            self.directed = True
            self.Make_Random_Point(self.NumOfNodes_ER.value())
            ER = nx.random_graphs.erdos_renyi_graph(self.NumOfNodes_ER.value(), self.ER_prob.value(), directed=True)
            xydata_b = self.b.get_offsets()
            pos = dict()
            for i in range(len(ER.nodes)):
                pos[i] = (xydata_b[i][0], xydata_b[i][1])
            ...
            xydata_b = self.b.get_offsets()
            for i in ER.edges():
                self.matplotlibwidget.axis.arrow(xydata_b[i[0]][0], xydata_b[i[0]][1],
                                                 xydata_b[i[1]][0] - xydata_b[i[0]][0],
                                                 xydata_b[i[1]][1] - xydata_b[i[0]][1],
                                                 width=0.02, head_width=0.12, length_includes_head=True,
                                                 zorder=2, fc='grey', ec='grey')
        self.G = ER
        self.hoverconnect = self.matplotlibwidget.figure.canvas.mpl_connect(
    ...
    def attack_by_bomb_continuous(self):
        ...
        self.img = Image.open('/Users/shitianyu/Desktop/urpn/pics/bomb.jpg')
    ...
    def draw_custom_result(self):
        ...
        if self.attack_method == 0:
            ...
            self.draw_answer_network(G, G0, 0, 0, 0, 0, xydata_b, False, "result")
        elif self.attack_method == 3:
            self.Confirm.setEnabled(False)
            if self.CustomNetwork is not None:
                self.Clear_Lines()
    ...
    def defend_start(self):
        ...
        self.matplotlibwidget_1.canvas.mpl_connect('button_press_event', self.on_click)
        self.matplotlibwidget_1.canvas.mpl_connect('button_release_event', self.off_click)

    def disconnect_bartool_2(self):
        self.Drag_2.setChecked(False)
        self.Color_2.setChecked(False)
        self.default()
